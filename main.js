...
const init_challenge = (div, func_loop, func_recursion, cleaner) => {
    ...
    input.oninput = () => {
        ...
        const [val_loop, val_rec] = clean_and_compute(func_loop, func_recursion, input.value, cleaner);
...
init_challenge(challenges[2], array_sum_loop, array_sum, (x) => x.split(",").map(Number));
